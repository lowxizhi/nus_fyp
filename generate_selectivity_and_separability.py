...
#     for loc1 in range(hp['n_loc']):
    for loc1 in [j*10 for j in range(int(hp['n_loc']/10))]:
...
#     for loc2 in range(hp['n_loc']):
    for loc2 in [j*10 for j in range(int(hp['n_loc']/10))]:
...
#         print('X_n shape: ' + str(X_n.shape))
        ...
        print('saved: ', performance_dict['trained accuracy on %s trials'%x.shape[0]])
        ...
        print('actual: ', round(custom_response_accuracy(y, model.predict(x)),4))
        
        with open(model_folder+f'/performance.json', 'w') as f:
            json.dump(performance_dict, f)
            
        print('changed: ', performance_dict['trained accuracy on %s trials'%x.shape[0]])
