...
def generate_trial(mode='random',batch_size=hp['batch_size_train'],**kwargs):
    ...
    if mode == 'random':
        ...
        stim1_locs_out = (stim1_locs + 2*np.pi/hp['n_loc']) % (2*np.pi)
        stim2_locs_out = (stim2_locs + 2*np.pi/hp['n_loc']) % (2*np.pi)
        ...
        stim1_locs = 2*np.pi*ind_stim_loc1/n_stim_loc1
        stim2_locs = 2*np.pi*ind_stim_loc2/n_stim_loc2
        stim1_locs_out = stim1_locs
        stim2_locs_out = stim2_locs
    ...
    # stim_locs = [stim1_locs_out[i] for i in range(batch_size)]
    # stim_locs2 = [stim2_locs_out[i] for i in range(batch_size)]
    # print(type(stim1_locs_out))
    # print(type(stim_locs))
    trial.add('out',stim1_locs_out,ons=fix_offs,offs=output_2_on)
    trial.add('out',stim2_locs_out,ons=output_2_on)
...
model_folder = '/hpctmp/e0316055/output_neighbour_experiment/output_neighbour/2_stim_batch_size_%s/n_hidden_%s/2_stim_batch_size_%s_n_hidden_%s_acc_%s_seed_%s'%(batch_size_train,n_rnn,batch_size_train,n_rnn,int(accuracy_threshold*100),seed)
if with_noise:
    hp['sigma_rec']=0.05
    hp['sigma_x']=0.01
