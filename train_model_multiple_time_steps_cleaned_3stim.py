...
def get_c_mask(batch_dim):
    ...
    stim2_ons =stim1_offs + int(1000/dt)
    stim2_offs = stim2_ons + int(300/dt)
    stim3_ons =stim2_offs + int(1000/dt)
    stim3_offs = stim3_ons + int(300/dt)
    fix_offs  = stim3_offs + int(1000/dt)
    output_2_on = fix_offs + int(500/dt)
    output_3_on = output_2_on + int(500/dt)
    tdim = output_3_on + int(500/dt)
...
def custom_response_accuracy(y_true, y_hat):
    ...
    if type(y_hat) is not np.ndarray:
        y_hat = y_hat.numpy()
    y_hat_loc1 = popvec(np.mean(y_hat[:, 220:245, 1:], axis=1))
    y_true_loc1 = popvec(np.mean(y_true[:, 220:245, 1:], axis=1))
    ...
    y_hat_loc2 = popvec(np.mean(y_hat[:, 245:270, 1:], axis=1))
    y_true_loc2 = popvec(np.mean(y_true[:, 245:270, 1:], axis=1))
    ...
    y_hat_loc3 = popvec(np.mean(y_hat[:, 270:, 1:], axis=1))
    y_true_loc3 = popvec(np.mean(y_true[:, 270:, 1:], axis=1))
    original_dist3 = y_true_loc3 - y_hat_loc3
    dist3 = np.minimum(abs(original_dist3), 2*np.pi-abs(original_dist3))
    corr_loc3 = dist3 < 2*np.pi/hp['n_loc']
    
    return np.sum(corr_loc1*0.333+corr_loc2*0.333+corr_loc3*0.333)/corr_loc1.shape[0]
...
def custom_response_loss(y_true, y_hat):
    ...
    return np.sum(dist1*0.333+dist2*0.333+dist3*0.333)/dist1.shape[0]*(360/(2*np.pi))
...
def generate_trial(mode='random',batch_size=hp['batch_size_train'],**kwargs):
    ...
    if mode == 'random':
        ...
        stim3_locs = rng.uniform(0, 2*np.pi, (batch_size,))

        stims_mean = rng.uniform(0.8,1.2,(batch_size,))
        stim_coh_range = np.array([0.,0.08,0.16,0.32])
        ...
        stims_coh  = rng.choice(stim_coh_range,(batch_size,))
        stims_sign = rng.choice([1,-1], (batch_size,))
        stim3_strengths = stims_mean + stims_coh*stims_sign

    elif mode == 'fixed':
        n_loc = kwargs['n_loc']
        batch_size = n_loc*n_loc*n_loc
        n_stim_loc1, n_stim_loc2, n_stim_loc3, repeat = stim_loc_shape = n_loc, n_loc, n_loc, 1
        stim_loc_size = np.prod(stim_loc_shape)
        ind_stim_loc1, ind_stim_loc2, ind_stim_loc3, ind_repeat = np.unravel_index(range(stim_loc_size),stim_loc_shape)
        stim1_locs = 2*np.pi*ind_stim_loc1/n_stim_loc1
        stim2_locs = 2*np.pi*ind_stim_loc2/n_stim_loc2
        stim3_locs = 2*np.pi*ind_stim_loc3/n_stim_loc3
    ...
    trial.add('stim', stim3_locs, ons=stim3_ons, offs=stim3_offs, strengths=stim3_strengths, mods=1)
    trial.add('fix_out', offs=fix_offs)
    stim_locs1 = [stim1_locs[i] for i in range(batch_size)]
    stim_locs2 = [stim2_locs[i] for i in range(batch_size)]
    stim_locs3 = [stim3_locs[i] for i in range(batch_size)]
    trial.add('out',stim_locs1,ons=fix_offs,offs=output_2_on)
    trial.add('out',stim_locs2,ons=output_2_on,offs=output_3_on)
    trial.add('out',stim_locs3,ons=output_3_on)
    ...
    trial.epochs = {'fix1':(None,stim1_ons),
                    ...
                    'delay2':(stim2_offs,stim3_ons),
                    'stim3':(stim3_ons,stim3_offs),
                    'delay3':(stim3_offs,fix_offs),
                    'go1':(fix_offs,output_2_on),
                    'go2':(output_2_on,output_3_on),
                    'go3':(output_3_on,None)}
    return trial
...
trial = generate_trial(mode='fixed',n_loc=32)
x = trial.x.swapaxes(0,1)
y = trial.y.swapaxes(0,1)
...
hp['n_epochs']=int(200000000/batch_size_train)
...
model_folder = '/hpctmp/e0316055/3_stim_batch_size_%s/n_hidden_%s/3_stim_batch_size_%s_n_hidden_%s_acc_%s_seed_%s'%(batch_size_train,n_rnn,batch_size_train,n_rnn,int(accuracy_threshold*100),seed)
if with_noise:
    hp['sigma_rec']=0.05
    hp['sigma_x']=0.01
...
dataset_train = tf.data.Dataset.from_generator(train_generator,
                                ...
                                output_shapes=((hp['batch_size_train'],x.shape[1],x.shape[2]),(hp['batch_size_train'],x.shape[1],x.shape[2])))
